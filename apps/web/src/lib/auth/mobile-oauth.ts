...
/** Validate and canonicalize the SHA-256 challenge generated by the native app. */
export function normalizeMobileHandoffChallenge(
  value: string | null | undefined
): string | null {
  if (value == null) {
    return null;
  }
  if (!MOBILE_HANDOFF_CHALLENGE_PATTERN.test(value)) {
...
export function buildMobileAuthCallbackUrl(
  ...
  const handoffChallenge = normalizeMobileHandoffChallenge(params.handoffChallenge);
  if (handoffChallenge) {
    url.searchParams.set("handoff_challenge", handoffChallenge);
  }
...
export function hashMobileHandoffVerifier(verifier: string): string {
...
export function buildMobileHandoffInsert(
  session: Session,
  code = createMobileHandoffCode(),
  challengeHash: string | null = null
) {
  const normalizedChallengeHash = normalizeMobileHandoffChallenge(challengeHash);
  return {
    code,
    row: {
      ...
      challenge_hash: normalizedChallengeHash,
