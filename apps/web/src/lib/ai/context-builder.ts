...
const CHAT_SURFACE_POLICY = [
  ...
  "- To send a DM call prepare_chat_message — pass person_query with the recipient's name or email. Only pass recipient_member_id when a real member UUID is already known from the current page or a tool result; never invent one. DMs do not require approval.",
...
export async function buildPromptContext(
  ...
  const systemPrompt = [
    ...
    `- Refusal format: reply briefly with exactly: "I can only help with TeamNetwork tasks for your organization — like members, events, announcements, discussions, jobs, contributions, or finding the right page. That request is outside what I do." Do not attempt a partial answer. Do not add a disclaimer then answer anyway. When you DO answer an in-scope question, never append this refusal sentence — the refusal text is only ever the entire response to an out-of-scope request.`,
    "- Greetings and small talk are fine — answer briefly and offer TeamNetwork-related examples.",
    "- Only refuse when the request is clearly unrelated to the organization. If it plausibly concerns members, alumni, events, announcements, discussions, jobs, mentorship, or other organization content, answer it instead of refusing.",
