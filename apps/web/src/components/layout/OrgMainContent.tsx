...
export function OrgMainContent({ children, hasTopBanner }: OrgMainContentProps) {
  ...
  return (
    <main
      id="main-content"
      tabIndex={-1}
      className={`scroll-mt-4 focus:outline-none lg:ml-[var(--sidebar-offset,3.5rem)] transition-[margin] duration-200 ease-in-out motion-reduce:transition-none ${reserveDrawerMargin ? "lg:mr-[420px]" : ""} ${isFullBleed ? "h-[calc(100dvh-4rem)] lg:h-dvh overflow-hidden pt-16 lg:pt-0" : "p-4 lg:p-8 pt-20 lg:pt-8"} ${hasTopBanner ? "mt-12" : ""}`}
    >
      {children}
    </main>
  );
