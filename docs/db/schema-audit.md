...
**Last Updated:** July 12, 2026 (Complete Table Reference and generated-type/OKF references reconciled; historical incident and timeline sections remain explicitly historical).
**Scope:** The repository contains 384 migration files through `20270110000000_add_notification_dismissals.sql`. The **Complete Table Reference** and AI/mobile companion links were refreshed against the current generated type dump and OKF bundle; migration files remain authoritative for exact policy and RPC state.
**Migration Count:** 384 (latest `20270110000000_add_notification_dismissals.sql`).
...
This document is a current-state schema index with historical notes. The generated types in `packages/types/src/database.ts` are the best day-to-day source of truth when this doc drifts; `apps/web/src/types/database.ts` is only a compatibility re-export. For per-policy RLS detail, grep the corresponding migration rather than duplicating SQL here.
...
| `knowledge_documents`    | Admin-curated org knowledge base (8th RAG source) | Admin-only RLS; `audience` (`all`/`admins`), `tags[]`. Included in `packages/types/src/database.ts` and the generated OKF bundle.                                                                                                        |
...
| `organization_email_domains`    | Per-org verified email sending domain (Resend)             | `domain`, `resend_domain_id`, `status`, `dns_records`, sender fields. Service-role RLS. Included in `packages/types/src/database.ts` and the generated OKF bundle. |
...
1. Prefer `packages/types/src/database.ts` when checking exact column names or enums; `apps/web/src/types/database.ts` is only a compatibility re-export.
...
1. **Header regeneration** — script that reads `supabase/migrations/` and rewrites the `Last Updated` / `Scope` / `Current Migration Count` block. Wire into `bun run gen:types`.
