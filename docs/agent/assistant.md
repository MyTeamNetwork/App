...
`get_engagement_metrics(window_days?)` returns windowed activity aggregates for an org. It queries the database directly for counts within the specified trailing window (default 30 days, max 365). Fields returned: `events_held`, `event_rsvps`, `announcements_published`, `discussion_threads_created`, `discussion_replies`, `feed_posts`, `feed_comments`, `chat_messages`, `active_contributors`. Any individual count can degrade to `null` if that query fails, without failing the whole tool. Use for questions like "how active was the org last month", "engagement trends", or "participation over the past 30 days". Steering: the `engagement_metrics` routing rule in `pass1-tools.ts` pre-empts `get_org_stats` for time-windowed engagement questions.
