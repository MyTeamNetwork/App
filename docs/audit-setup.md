...
The repository still contains audit helper scripts under `scripts/audit/` and audit-specific environment variables in `playwright.config.ts`, but the old `audit:*` wrappers are no longer present in `package.json`.
