-- Fix: revoked-user branch in redeem_parent_invite was missing the parents-table upsert.
-- A user reactivated from a revoked non-parent role was being written to
-- user_organization_roles with role='parent' but no corresponding row was inserted
-- into public.parents, causing them to be invisible in the parents directory and
-- causing parent quota counts to undercount.
--
-- Fix: insert the same 3-step parents upsert block (already present in the new-user
-- path) into the revoked-user branch before its RETURN.
