-- Stop deriving org-admin from the members.role job-title field.
--
-- members.role is free-form "Role/Position" text (see newMemberSchema in
-- apps/web/src/lib/schemas/member.ts — optionalSafeString(100), same tier as
-- current_company/school), and enrichment backfills it from external job-title
-- data. The previous sync mapped a job title literally equal to "admin" into
-- user_organization_roles.role = 'admin' — an active, unaudited org-admin
-- grant on the user's next login. Admin must only ever be granted through the
-- audited execute_member_role_change RPC or the enterprise invite flow.
--
-- Synced members now always land as 'active_member'; the alumni/parent
-- mappings are unchanged (those enum-like values gate access down, not up).
